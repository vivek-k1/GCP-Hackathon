...
class SimulateRequest(BaseModel):
    session_id: str
    side: str = "accuser"          # which side the turn belongs to
    manual_argument: Optional[str] = ""  # if set, `side` is the user's side


@app.post("/case/analyze")
async def case_analyze(req: AnalyzeRequest):
    """Senior Lawyer Agent: classify domain + spot legal issues, open a session."""
    from app.courtroom import analyze_case, create_session

    facts = (req.case_facts or "").strip()
    if len(facts) < 30:
        raise HTTPException(status_code=400, detail="Please describe the case in a little more detail (min ~30 characters).")
    try:
        analysis = analyze_case(facts)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Case analysis failed: {e}")
    session = create_session(facts, analysis)
    return {"session_id": session.id, "analysis": analysis}


@app.post("/kanoon/retrieve")
async def kanoon_retrieve(req: RetrieveRequest):
    """Retriever Agent: query Indian Kanoon and build a local precedent vector index."""
    from app.courtroom import get_session, run_retrieval
    from app.kanoon_client import get_kanoon_client

    session = get_session(req.session_id)
    if session is None:
        raise HTTPException(status_code=404, detail="Unknown session_id. Run /case/analyze first.")
    try:
        precedents = run_retrieval(session, court=req.court or "", max_results=req.max_results)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Precedent retrieval failed: {e}")
    return {
        "session_id": session.id,
        "kanoon_live": get_kanoon_client().live,
        "precedents": precedents,
        "indexed": session.retriever is not None,
    }


@app.post("/courtroom/simulate-turn")
async def courtroom_simulate_turn(req: SimulateRequest):
    """Courtroom state machine: stream the next argument (SSE) + updated jury verdict."""
    from app.courtroom import get_session, simulate_turn_stream
    ...
    def event_stream():
        try:
            for event in simulate_turn_stream(
                session,
                side=req.side,
                manual_argument=req.manual_argument or "",
            ):
...
@app.get("/courtroom/jury-verdict")
async def courtroom_jury_verdict(session_id: str = Query(...)):
    """Jury Evaluator: validate citations + compute the S_jury propensity score."""
    from app.courtroom import get_session, compute_jury_verdict

    session = get_session(session_id)
    if session is None:
        raise HTTPException(status_code=404, detail="Unknown session_id.")
    try:
        return compute_jury_verdict(session)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Jury scoring failed: {e}")


@app.get("/courtroom/session/{session_id}")
async def courtroom_session_state(session_id: str):
    """Resync helper: current transcript + precedents + analysis for a session."""
